...
from utils.Utils import joint_val_image, postprocessing, save_per_img_gt
...
def main():
    ...
    parser.add_argument('--model-file', type=str, default='', help='Model path')
    ...
    parser.add_argument('--test-prediction-save-path', type=str, default='', help='Path root for test image and mask')
    ...
    for batch_idx, (sample) in tqdm.tqdm(enumerate(test_loader),total=len(test_loader),ncols=80, leave=False):
        ...
        for i in range(prediction.shape[0]):
            ...
            for img, lt, lp in zip([imgs[i]], [target_numpy[i]], [prediction_post]):
                ...
                save_per_img_gt(img.numpy().transpose(1, 2, 0),
                             output_path,
                             img_name[i],
