...
def main():
    ...
    parser.add_argument('--datasetTrain', nargs='+', type=int, default=[1,4,3], help='train folder id contain images ROIs to train range from [1,2,3,4]')
    ...
    #/media/hansheng/Seagate Expansion Drive/Seagate/Registration/DG/dataset/dataset/Fundus-doFE/Fundus
    parser.add_argument('--data-dir', default='', help='data root path')
    parser.add_argument('--pretrained-model', default='.', help='pretrained model of FCN16s',)
    parser.add_argument('--out-stride', type=int, default=16, help='out-stride of deeplabv3+',)
    parser.add_argument('--interval_validate', type=int, default=1, help='interval validate', )
    args = parser.parse_args()

    now = datetime.now()
    args.out = osp.join(local_path, 'logs_all', 'test'+str(args.datasetTest[0]), 'lam'+str(args.lam), now.strftime('%Y%m%d_%H%M%S.%f'))
    args.log_out=osp.join('','logs_all', 'test'+str(args.datasetTest[0]), 'lam'+str(args.lam), now.strftime('%Y%m%d_%H%M%S.%f'))
    ...
    # 2. model
    model = DeepLab(num_classes=2, num_domain=3, backbone='mobilenet', output_stride=args.out_stride, lam=args.lam,style_mode=True).cuda()
