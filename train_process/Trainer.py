...
class Trainer(object):
    ...
    def __init__(self, cuda, model, lr, val_loader, train_loader, out, log_out,max_epoch, optim, stop_epoch=None,
        ...
        if not osp.exists(osp.join(self.log_out, 'log.csv')):
            with open(osp.join(self.out, 'log.csv'), 'w') as f:
                f.write(','.join(self.log_headers) + '\n')

        log_dir = os.path.join(self.log_out, 'tensorboard',
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(
                ...
                with torch.no_grad():
                    predictions, domain_predict, _,_,_ = self.model(data)
            ...
            self.writer.add_scalar('val_data/loss_cls', loss_cls, self.epoch * (len(self.train_loader)))
            ...
            if is_best:
                ...
                }, osp.join(self.out, 'checkpoint_best_%d.pth.tar' % self.best_epoch))
            else:
    ...
    def train_epoch(self):
        ...
        self.running_content_loss = 0.0
        self.running_style_loss = 0.0
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            output,domain_predict, output_style,domain_predict_style, idx_swap = self.model(image)
            loss_seg = bceloss(torch.sigmoid(output), target_map)
            loss_cls = 0.1 * mseloss(softmax(domain_predict), domain_code)
            loss_content = bceloss(torch.sigmoid(output_style), target_map) + 0.1 * mseloss(output, output_style)
            loss_style = 0.1 * mseloss(softmax(domain_predict_style), domain_code[idx_swap])

            self.running_seg_loss += loss_seg.item()
            self.running_content_loss += loss_content.item()
            self.running_style_loss += loss_style.item()
            self.running_cls_loss += loss_cls.item()
            loss_data = (loss_cls + loss_seg + loss_content + loss_style).data.item()

            if np.isnan(loss_data):
                raise ValueError('loss is nan while training')

            loss = loss_content+ loss_seg+loss_cls+loss_style
            loss.backward()
            self.optim.step()
            ...
            if iteration % 30 == 0:
                ...
                grid_image = make_grid(torch.sigmoid(output_style)[0, 0, ...].clone().cpu().data, 1, normalize=True)
                self.writer.add_image('train/prediction_cup_style', grid_image, iteration)
                grid_image = make_grid(torch.sigmoid(output_style)[0, 1, ...].clone().cpu().data, 1, normalize=True)
                self.writer.add_image('train/prediction_disc_style', grid_image, iteration)
            ...
            self.writer.add_scalar('train_gen/loss_content', loss_content.data.item(), iteration)
            self.writer.add_scalar('train_gen/loss_style', loss_style.data.item(), iteration)

        self.running_seg_loss /= len(self.train_loader)
        self.running_cls_loss /= len(self.train_loader)
        self.running_content_loss /= len(self.train_loader)
        self.running_style_loss /= len(self.train_loader)
